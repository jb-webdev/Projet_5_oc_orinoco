...
//===================== je crée une requête api pour récupèrer les données du produit ==========================
let urlApiProd = "http://localhost:3000/api/cameras"+"/"+ id;  // on déclare la variable pour l'url choisi.

const getProduit = function promiseApiProduit () {              // on declare une constante et on rentre une fonction promiseApi.

    return new Promise((resolve, reject) => {      // on lui donne deux fonctions resolve si elle tiens la promesse et reject si elle ne tiens pas ca promesse
        const xhrEssai = new XMLHttpRequest();      // on utilise la méthode Ajax avec XMLRTTpRequest.
        ...
        xhrEssai.onreadystatechange = function() {
            if (this.readyState == XMLHttpRequest.DONE) { // on lui passe les conditions avec if.
                ...
                // console.log(this.responseText);         // // on vérifie la réponse du fichier JSON.
                } else {
    ...
    })  // "84d6ae60-da42-11ea-a1ea-6b632e4d7dbc"
};
getProduit(urlApiProd).then(function(response) {
